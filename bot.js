...
async function check() {
    try {
        ...
        if (!data) return;
        const { balances, pnl, fees, deposit, pools } = data;

        const poolNames = pools.map(p => `${p.tokenX}/${p.tokenY}`).join(", ");
        console.log(`💰 Valeur: $${balances.toFixed(2)} | PnL: $${pnl.toFixed(2)} | Fees: $${fees.toFixed(2)} | Pools: ${poolNames}`);

        if (pnl >= ALERT_THRESHOLD) {
            if (lastAlertPnl === null || pnl >= lastAlertPnl + ALERT_THRESHOLD) {
                bot.sendMessage(CHAT_ID,
                    `🚨 SORTIR DE POSITION ?\n\n` +
                    `💰 Valeur: $${balances.toFixed(2)}\n` +
                    `📈 PnL: +$${pnl.toFixed(2)}\n` +
                    `💸 Fees non réclamées: $${fees.toFixed(2)}\n` +
                    `🏊 Pools: ${poolNames}\n\n` +
